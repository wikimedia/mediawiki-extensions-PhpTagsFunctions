...
class PhpTagsFunctions_PCRE_Test extends \PHPUnit_Framework_TestCase {
...
print_r( preg_filter($pattern, $replace, $subject) );');
		$this->assertEquals(
				(string) new outPrint( null, print_r(array('A:C:1', 'B:C:a', 'A:2', 'B:b', 'A:3', 7=>'A:4'), true) ),
				(string) $return[0]
			);
	}
	public function testRun_preg_replace_1() {
		...
		$this->assertEquals(
				(string) new outPrint( null, print_r(array('A:C:1', 'B:C:a', 'A:2', 'B:b', 'A:3', 'A', 'B', 'A:4'), true) ),
				(string) $return[0]
			);
	}
...
print_r( preg_grep("/^(\d+)?\.\d+$/", $array) );');
		$this->assertEquals(
				(string) new outPrint( null, print_r(array(2=>'4.78', 4=>'7.89'), true) ),
				(string) $return[0]
			);
	}
...
echo preg_replace($patterns, $replacements, $string);'),
	...
	public function testRun_preg_split_1() {
		$this->assertEquals(
				...
				array( print_r( array('hypertext', 'language', 'programming'), true ) )
			);
	}
...
echo print_r($chars,true);'),
				array( print_r( array('s', 't', 'r', 'i', 'n', 'g'), true ) )
			);
	}
...
echo print_r($chars, true);'),
				array( print_r( array( array('hypertext', 0), array('language', 10), array('programming', 19) ), true ) )
			);
	}
